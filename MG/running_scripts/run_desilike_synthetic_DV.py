...
def plot_spectrum(new_likelihoods: ObservablesGaussianLikelihood):
    ...
    for tracer, color, new_likelihood in zip(tracers, colors, new_likelihoods):
        ...
        for covsyst in ['rotation-hod-photo']:
            ...
            # for param in new_likelihood.all_params.select(basename=['alpha*shotp']):
            for param in new_likelihood.all_params.select(basename=['alpha0p', 'alpha2p', 'alpha4p']):
                if param.varied:
                    param.update(derived='.best')
            logpost, derived = new_likelihood(params, return_derived=True)
            for name in derived.names(solved=True):
                print(f"Updating {name} to {derived[name]} from {params.get(name, 'N/A')}")
                params[name] = float(derived[name])
            print('fkpt参数更新后')
...
def parse_args():
    ...
    p.add_argument("--mode", choices=["mcmc", "emcee", "map", 'NUTS', 'HMC', 'Nautilus', 'plot_spectrum'], default="mcmc")
    ...
    p.add_argument(
        "--solve",
        choices=[".marg", ".marg_not_derived", ".best"],
        default=".marg",
    )
